...
const EnhancedMapComponent = ({
  ...
  // Initialize map with performance optimizations and better error handling
  useEffect(() => {
    ...
    const initializeMap = async () => {
      try {
        ...
        mapLoadTimeout.current = setTimeout(() => {
          if (!mapInitialized) {
            console.warn('Map loading timeout - attempting recovery');
            setMapError('Map is taking longer than expected to load. Please refresh the page.');
          }
          ...
          transformRequest: (url, resourceType) => {
            if (resourceType === 'Tile') {
              return {
                url: url,
                headers: {
                  'Cache-Control': 'max-age=300' // 5 minute cache for tiles
                }
              };
            }
            return { url };
  ...
  }, [isDarkMode, mapInitialized, onMapLoad, boundsEqual, updateVisibleLayers, HALIFAX_CENTER]);
  ...
  }, [enableCaching, getLayerConfig, getLayerFileName]);
  ...
  }, [maxFeaturesPerLayer, simplifyTolerance, enableClustering, loadLayerData, addLayerToMap]);
  ...
  }, [visibleLayers, activeLayers, enableLazyLoading, loadingLayers, processLayerQueue, map]);

  // Update route with performance optimizations
  useEffect(() => {
    console.log('EnhancedMapComponent: Route update triggered', {
      hasMap: !!map.current,
    ...
    if (!map.current || !map.current.isStyleLoaded()) {
      console.log('EnhancedMapComponent: Skipping route update - map not ready');
      return;
    }
    
    // Create a test route if no route is provided (for debugging)
    const routeToUse = route || {
      ...
      features: [{
        ...
        geometry: {
          type: 'LineString',
          coordinates: [
            [-63.5756, 44.6475], // Halifax center
            [-63.5700, 44.6500], // Slightly north
            [-63.5650, 44.6525]  // Further north
          ]
        },
        properties: {
          distance: 1000,
    ...
    if (!routeToUse) {
      console.log('EnhancedMapComponent: No route data available');
      return;
    }
    ...
    const updateRoute = () => {
      try {
        ...
        let routeData = routeToUse;
        
        console.log('EnhancedMapComponent: Processing route data:', routeData);
        ...
        console.log('EnhancedMapComponent: First feature:', firstFeature);
        ...
        console.log('EnhancedMapComponent: Route geometry coordinates:', firstFeature.geometry.coordinates);
        ...
        console.log('EnhancedMapComponent: Adding route source:', routeSource);
        
        map.current.addSource(routeSourceId, routeSource);
        
        console.log('EnhancedMapComponent: Route source added successfully');

        // Add route layer with improved visibility
        const routeLayer = {
          id: routeId,
          type: 'line',
          ...
          paint: {
            ...
            'line-width': 20, // Thicker line for debugging
            'line-opacity': 1.0
          }
        };
        
        console.log('EnhancedMapComponent: Adding route layer:', routeLayer);
        
        map.current.addLayer(routeLayer, map.current.getLayer('poi-label') ? 'poi-label' : undefined);
        
        console.log('EnhancedMapComponent: Route layer added successfully');
        console.log('EnhancedMapComponent: All map layers:', map.current.getStyle().layers.map(l => l.id));
        
        // Test if the route is visible by checking the layer
        setTimeout(() => {
          if (map.current.getLayer(routeId)) {
            console.log('EnhancedMapComponent: Route layer is present and should be visible');
            console.log('EnhancedMapComponent: Route layer paint properties:', map.current.getPaintProperty(routeId, 'line-color'));
            console.log('EnhancedMapComponent: Route layer layout properties:', map.current.getLayoutProperty(routeId, 'visibility'));
          } else {
            console.error('EnhancedMapComponent: Route layer not found after adding');
          }
        }, 100);
        ...
        map.current.addLayer({
          ...
          paint: {
            'line-color': '#000000', // Black border for debugging
            'line-width': 24, // Thicker border for debugging
            'line-opacity': 1.0
          }
        }, routeId);
        ...
        if (routeData.features && routeData.features.length > 0) {
          ...
          console.log('EnhancedMapComponent: Route coordinates:', coordinates);
