...
const BasicMapComponent = ({
  ...
  useEffect(() => {
    ...
    // ALWAYS remove old accessible parking layer (green circles) when in driving mode
    // In driving mode, we use parkingMarkersService for blue 🅿️ markers instead
    if (isDrivingRoute) {
      if (map.current.getLayer("accessible-parking-layer")) {
        console.log('🧹 [DRIVING MODE] Removing old accessible parking layer (green circles)');
      ...
      // Exit early - don't add green circles in driving mode
      return;
    }
    ...
    const renderRouteWithDelay = () => {
      setTimeout(async () => {
        if (map.current && map.current.isStyleLoaded && map.current.isStyleLoaded()) {
          ...
          // Fit map to route bounds - with validation to prevent jumping to invalid locations
          if (routeData.features && routeData.features.length > 0) {
            ...
            if (coordinates && coordinates.length > 0) {
              ...
              // Only fit bounds if we have valid coordinates within Halifax
              if (validCoordinates.length > 0) {
                ...
                console.log('✅ Map fitted to valid route bounds within Halifax');
              } else {
                console.warn('⚠️ Route coordinates are outside Halifax bounds, keeping map centered on Halifax');
